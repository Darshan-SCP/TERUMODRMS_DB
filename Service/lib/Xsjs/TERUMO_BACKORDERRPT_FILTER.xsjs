...
function _passFiltersToViewReport(aFilterResponse, oPostInputObj) {
	...
	query = 'SELECT  \"Division\",\"SubDivision\",\"Region\",\"SA\",\"SM\",\"RH\",\"CustomerCode\",' +
		...
		'\"OrderQuantity\",\"OrderUnitPrice\",\"Discount\",\"OrderNetValue\",\"ReqQty\",\"FullFillQty\" ' +
...
function _fetchData(eQuery) {
	...
	while (rs.next()) {
		...
		record.ReqQty = rs.getInt(18);
		record.FullFillQty = rs.getInt(19);

		if (record.OrderQuantity === 0 || record.ReqQty !== record.FullFillQty) {
			record.OrderQuantity = record.ReqQty - record.FullFillQty;
			record.OrderNetValue = (record.OrderUnitPrice - record.Discount) * record.OrderQuantity;
		}
		
		//Change for removing Completed Line Items - Kaustubh 20.06.2022
		if (record.ReqQty === record.FullFillQty) {
		continue;
		}
		else
		{
